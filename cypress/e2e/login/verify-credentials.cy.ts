...
describe('Login form', () => {
  ...
  it('calls verifyCredentials', () => {
    ...
    const verify = cy.stub().as('verify').returns(true)
    ...
    cy.visit('/', {
      onBeforeLoad(win) {
        win.VALID_USERNAMES = [username]
        win.verifyCredentials = verify
      },
    })
    ...
    cy.get('@verify').should('have.been.calledOnceWith', username, password)
  })
})
